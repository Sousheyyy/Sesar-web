...
import { ArrowLeft, Music2, TrendingUp, Users, Clock, Calendar, Target, Zap, DollarSign, Info } from "lucide-react";
...
export default async function AdminCampaignDetailPage({
  ...
  let remainingScore = totalCampaignScore;
  ...
  // Simple approach: Cap high earners at 40% of the pool, distribute remainder among others
  // Note: This is a simplified version of the full recursive redistribution logic.
  // For the UI, we will show the "estimated" values based on this pass.
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">{campaign.song.title} - {campaign.artist.name}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <AdminCampaignHeaderActions campaignId={campaign.id} currentStatus={campaign.status} />
          <FinishCampaignButton campaignId={campaign.id} campaignStatus={campaign.status} />
        </div>
      </div>
        ...
        <Card className="overflow-hidden">
          ...
          <CardContent className="space-y-0 pb-4">
            ...
            <p className="text-xs text-muted-foreground mb-1">
              Dağıtılabilir (Ücretler sonrası)
            </p>
            <p className="text-xs text-muted-foreground pt-1 border-t">
              Brüt: {formatCurrency(Number(campaign.totalBudget))}
            </p>
          </CardContent>
        </Card>
      ...
      {/* Side-by-side: Music Stats and Approval Section */}
      <div className="grid gap-6 md:grid-cols-2">
        {campaign.song.tiktokMusicId ? (
          ...
          // Placeholder if no music stats available, so grid layout is maintained or handled
           <Card>
             <CardHeader>
